...
try:
    ...
    try:
        ...
        Because you are using django <1.4, you need to install six.
        """
...
class CoverageRunner(DjangoTestSuiteRunner):
    ...
    def run_tests(self, test_labels, extra_tests=None, **kwargs):
        ...
        if settings.COVERAGE_USE_STDOUT:
            ...
            if excludes:
                message = "\nThe following packages or modules were excluded: %s\n" % " ".join(excludes)
                six.print_(message, file=sys.stdout)
            if errors:
                message = ("\nThere were problems with the following packages \n"
                           "or modules:%s\n" % " ".join(errors))
                six.print_(message, file=sys.stdout)
        ...
        if outdir:
            ...
            six.print_("\nHTML reports were output to '%s'" %outdir, file=sys.stdout)
